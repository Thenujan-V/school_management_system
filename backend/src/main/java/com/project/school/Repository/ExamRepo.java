...
public interface ExamRepo extends JpaRepository<ExamEntity, Integer> {
    @Query("select s.grade, s.term, s.subject_name, e.exam_id, e.exam_date, e.exam_time, e.exam_hall from ExamEntity e join SubjectEntity s on e.subject_id = s.subject_id where s.grade = :grade and s.term = :term")
